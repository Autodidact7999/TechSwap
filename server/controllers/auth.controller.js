...
const addContextData = async (req, res, next) => {

  console.log("Adding context data for user: ", req.email || "unknown")
  ...
  try {
    ...
    console.log('Context data saved successfully for user:', email);
    
    res.status(200).json({ message: "Email verified successfully, context added." });
  } catch (error) {
    console.error('Failed to save context data for user:', email, error);
    res.status(500).json({
      message: "Internal server error while saving context data",
    });
  }
};
