...
const signin = async (req, res, next) => {
  ...
  try {
      ...
      console.log("Password correct, checking for context-based auth settings...");
      const isContextAuthEnabled = await UserPreference.findOne({ user: existingUser._id, enableContextBasedAuth: true });

      if (isContextAuthEnabled) {
          console.log("Context-based authentication is enabled, verifying context data...");
          const contextDataResult = await verifyContextData(req, existingUser);

          console.log("Context data result:", contextDataResult);
          if (contextDataResult === types.BLOCKED) {
              console.log("Access blocked for suspicious activity for:", email);
              await saveLogInfo(req, MESSAGE.DEVICE_BLOCKED, LOG_TYPE.SIGN_IN, LEVEL.WARN);
              return res.status(401).json({ message: "You've been blocked due to suspicious login activity. Please contact support for assistance." });
          }

          if (contextDataResult === types.NO_CONTEXT_DATA || contextDataResult === types.ERROR) {
              console.log("Error or missing context data for:", email);
          ...
          if (contextDataResult === types.SUSPICIOUS) {
              console.log("Suspicious activity detected for:", email);
              await saveLogInfo(req, MESSAGE.MULTIPLE_ATTEMPT_WITHOUT_VERIFY, LOG_TYPE.SIGN_IN, LEVEL.WARN);
              return res.status(401).json({
                  message: "You've temporarily been blocked due to suspicious login activity. Please verify your identity via email."
              });
          }
      }
      ...
      const newRefreshToken = new Token({ user: existingUser._id, refreshToken, accessToken });
 ...
 *
 * @description If the email domain of the user's email is "mod.TechSwap.com", the user will be
...
const addUser = async (req, res, next) => {
  console.log("Received request to add user with email:", req.body.email); // Log email on user creation attempt
  ...
  try {
    ...
    console.log("Password hashed successfully for user:", req.body.email);

    // Parse consent given from request
    const isConsentGiven = req.body.isConsentGiven === "true";

    // Determine role based on email domain
    const emailDomain = req.body.email.split("@")[1];
    const role = emailDomain === "mod.techswap.com" ? "moderator" : "general";
    ...
    // Check if consent was not given, respond accordingly
    if (!isConsentGiven) {
      return res.status(201).json({
        message: "User added successfully without context-based authentication enabled."
      });
    } else {
    ...
    if (req.files && req.files.length > 0) {
      ...
      const filePath = `${req.protocol}://${req.get("host")}/assets/userAvatars/${req.files[0].filename}`;
      console.log("Attempting to delete uploaded file due to error:", filePath);
    }

    return res.status(400).json({
      message: "Failed to add user due to error: " + err.message
    });
  }
};
